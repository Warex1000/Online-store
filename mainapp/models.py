...
ContentType Микрофрейм ворк который видет модели которые есть в INSTALLED_APPS в settings предоставляя универсальный интерфейс
"""
...
class Category(models.Model):
    ...
    slug = models.SlugField(unique=True)  #короткое название-метка, содержит только буквы числа нижнее подчеркивание дефис. Используются в URL(Category/nootebook)
...
class Product(models.Model):
    ...
    category = models.ForeignKey(Category, verbose_name='Категория', on_delete=models.CASCADE)  #Обект Category класса Product наследуеться(связан) от класса Category / при удалении обекта удаляеться все связи с ним (данные)
    ...
    price = models.DecimalField(max_digits=9, decimal_places=2, verbose_name='Цена')  #Параметры цены, максимальное колличество символов до запятой(9) и после(2)
...
class CartProduct(models.Model):
    ...
    content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE)  #Микрофрейм ворк который видет модели которые есть в INSTALLED_APPS в settings предоставляя универсальный интерфейс
    object_id = models.PositiveIntegerField()  # Индификатор инстанс этой модели
    content_object = GenericForeignKey('content_type', 'object_id')
    qty = models.PositiveIntegerField(default=1)   #Колличество выбранного товара в корзине, по умолчанию 1, Подобно IntegerField, но должно быть либо положительным, либо нулевым (0). Значения от 0 до 2147483647 безопасны во всех базах данных, поддерживаемых Django.
    final_price = models.DecimalField(max_digits=9, decimal_places=2, verbose_name='Цена')   #Финальная цена всех товаров в корзине

    def __str__(self):
        return "Продукт: {} (для корзины)".format(self.product.title)   #!!! Неразрешенная ссылка на атрибут "title" для класса "ForeignKey"


class Cart(models.Model):

    owner = models.ForeignKey('Customer', verbose_name='Владелец', on_delete=models.CASCADE)
    products = models.ManyToManyField(CartProduct, blank=True, related_name='related_cart')   #Связь многие ко многим к CartProduct
